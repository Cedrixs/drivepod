...
import { CenteredSpinner, EmptyState } from './primitives';
import type { DashboardStats } from '../state/listeningStats';

function fmt(minutes: number): string {
  return formatMinutes(minutes) ?? '0 min';
}
...
export function Dashboard(): React.JSX.Element {
  ...
  useEffect(() => {
    let cancelled = false;
    void getAllListeningDays().then((days) => { if (!cancelled) setStats(computeStats(days)); });
  ...
  return (
    <div style={{ padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        <StatCard label="Série" value={`${stats.streak} j`} />
            ...
            {sources.map(([src, min]) => (
              <div key={src}>
                ...
                <div style={{ height: 3, background: 'var(--surface-3)', borderRadius: 2, overflow: 'hidden' }}>
                  <div style={{ height: '100%', background: 'var(--accent)', borderRadius: 2, width: `${Math.round((min / maxMin) * 100)}%` }} />
                </div>
              </div>
            ))}
          </div>
      ...
      {stats.weekMinutes === 0 && sources.length === 0 && (
        <EmptyState title="Commencez à écouter pour voir vos statistiques" padding={32} />
      )}
    </div>
  );
}
